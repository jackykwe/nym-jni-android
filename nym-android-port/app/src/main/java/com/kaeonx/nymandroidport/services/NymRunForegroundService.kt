...
class NymRunForegroundService : Service() {
    ...
    private fun getNetworkStatistics(): String {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            return "Network Statistic (SDKerr) | SDK too low (${Build.VERSION.SDK_INT} < ${Build.VERSION_CODES.M})"
        }
...
//            val transportInfo = activeNetworkCapabilities.transportInfo as WifiInfo
            // Deprecated as of API 31 but still useable
            val transportInfo = wifiManager.connectionInfo
...
//            "Network Statistic (WiFi) | ssid='$ssid' standard=$wifiStandard lsMbps=$linkSpeedMbps rxLsMbps=$rxLinkSpeedMbps txLsMbps=$txLinkSpeedMbps dBmRssi=$dBmRssi signalLevel=$signalLevel maxSignalLevel=$maxSignalLevel"
            ...
            val dataNetworkType = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                when (val dnt = telephonyManager.dataNetworkType) {
            ...
            // signalStrength(): Due to power saving this information may not always be current.
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                logFragments.add("SDK<${Build.VERSION_CODES.Q}")
            }
